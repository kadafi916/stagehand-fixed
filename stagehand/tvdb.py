...
class Episode:
    ...
    def preferred_filename(self):
        ...
        # bytes
        fmt = fsname(config.naming.episode_format)
        dt = self.airdate
        if self.series.cfg.identifier == 'date' and dt:
            style = fsname(config.naming.date_style)
            code = dt.strftime(style)
        else:
            style = fsname(config.naming.code_style)
            code = style.format(season=self.season.number, episode=self.number)
        return fmt.format(show=fsname(fixsep(self.series.name)), code=code, title=fsname(fixsep(self.name)))
...
class TVDB(db.Database):
    ...
    def __init__(self, dbfile, *, loop=None):
        super().__init__(dbfile, {
            # Module map for unpickler, conversion from python2 to python3.
            # Old objects in the database may use date objects from kaa which
            # are now obsoleted.
            'kaa.base.dateutils': 'stagehand.toolbox.dateutils'
        }, loop=loop)
        self._version = 0
        self._series_cache = {}
        ...
        self.register_object_type_attrs('series',
            provider = (str, db.ATTR_SEARCHABLE),
            conflict = (int, db.ATTR_SEARCHABLE),
            conflict_info = (dict, db.ATTR_SIMPLE),
            imdbid = (str, db.ATTR_SEARCHABLE),
            name = (str, db.ATTR_SEARCHABLE | db.ATTR_INVERTED_INDEX, 'keywords'),
            status = (int, db.ATTR_SEARCHABLE),
            started = (str, db.ATTR_SEARCHABLE),  # YYYY-MM-DD
            runtime = (int, db.ATTR_SEARCHABLE),
            airtime = (str, db.ATTR_SEARCHABLE),
            overview = (str, db.ATTR_SEARCHABLE | db.ATTR_INVERTED_INDEX, 'keywords'),
            banner = (str, db.ATTR_SEARCHABLE),
            banner_data = (db.RAW_TYPE, db.ATTR_SIMPLE),
            poster = (str, db.ATTR_SEARCHABLE),
            poster_data = (db.RAW_TYPE, db.ATTR_SIMPLE),
            genres = (list, db.ATTR_SIMPLE | db.ATTR_INVERTED_INDEX, 'genres')
        )

        self.register_object_type_attrs('episode',
            name = (str, db.ATTR_SEARCHABLE | db.ATTR_INVERTED_INDEX, 'keywords'),
            overview = (str, db.ATTR_SEARCHABLE | db.ATTR_INVERTED_INDEX, 'keywords'),
            season = (int, db.ATTR_SEARCHABLE),
            episode = (int, db.ATTR_SEARCHABLE),
            airdate = (str, db.ATTR_SEARCHABLE),
            status = (int, db.ATTR_SEARCHABLE),
            filename = (str, db.ATTR_SEARCHABLE),
            search_result = (SearchResult, db.ATTR_SIMPLE),
            # A list of SearchResults that have been blacklisted (user decided
            # the result was not appropriate for this episode).
            blacklist = (list, db.ATTR_SIMPLE),
            keywords = (list, db.ATTR_SIMPLE | db.ATTR_INVERTED_INDEX, 'keywords')
        )
    ...
    def _add_or_update(self, type, idmap, addattrs, **kwargs):
        ...
        if obj:
            if type == 'series':
                log.debug('updating series object: idmap=%s', idmap)
            self.update(obj, **kwargs)
        else:
            kwargs.update(addattrs)
            if type == 'series':
                log.debug('creating new series object: idmap=%s', idmap)
    ...
    def _get_conflicts(self, pseries):
        ...
        # features).
        n_real_conflicts = 0
        for (nn, airdate), episodes in list(conflicts.items()):
            if nn is None:
        ...
        categorized = sum(len(eplist) for eplist in itertools.chain(matches.values(), conflicts.values())) + len(unmatched)
        # Raw count ignore eps with no name
        raw = sum(1 for series in pseries.values() for ep in series['episodes'] if ep['name'])
        log.debug('n_real_conflicts=%d matches=%d conflicts=%d unmatched=%d max=%d categorized=%d raw=%d' % (n_real_conflicts, len(matches), len(conflicts), len(unmatched), n_episodes, categorized, raw))
        if categorized != raw:
            alleps = list(itertools.chain(*[series['episodes'] for series in pseries.values()]))
            log.error('_get_conflicts() lost episodes', len(alleps))
            for eplist in matches.values() + conflicts.values():
                for p, ep in eplist:
                    try:
                        alleps.remove(ep)
                    except ValueError:
                        log.debug('not missing: %s', ep)
            for p, nn, ep in unmatched:
                alleps.remove(ep)
            for ep in alleps:
                log.debug('missing: %s', ep)
    ...
    async def _do_update_series(self, provider, id, dirty=[], preferred=None, fast=False, completed=None):
        ...
        :param completed: a Signal object that will be emitted once finished
                          (this is mainly used internally for fast=True)
        :returns: None if the series is fully updated, or a Signal object if
        ...
        assert(not isinstance(provider, str) and not isinstance(preferred, str))
        ...
        existing = self.query_one(type='series', **{provider.IDATTR: tostr(id)})
        if existing:
    ...
    def _update_db_with_pseries(self, pseries, preferred):
        ...
        if len(pseries) > 1:
            ...
            episodes = []
            for eplist in itertools.chain(conflicts.values(), matches.values()):
                ...
                if max(counts.values()) > 1:
                    ...
                    merged = {}
                    for p, ep in sorted(eplist, key=lambda xep: xep[0] != preferred):
                        for k, v in ep.items():
        ...
        # orphaned rows.
        orphans = self.query(type='episode', parent=parent, id=db.QExpr('not in', dbids))
        for orphan in orphans:
            ...
            dupes = self.query(type='episode', parent=parent, id=db.QExpr('!=', orphan['id']),
    ...
    async def add_series_by_id(self, id, fast=True):
        ...
        .. note::
           This function returns after the preferred provider gives results.
           Other providers are consulted in the background and the database
           will be updated asynchronously to reflect the other providers.

           Currently there is no notification when the non-preferred providers
