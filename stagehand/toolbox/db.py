...
class PyObjectRow:
    ...
    __slots__ = ('_description', '_object_types', '_type_name', '_row', '_pickle',
                '_idxmap', '_typemap', '_keys', '_modmap')
    def __init__(self, cursor, row, pickle_dict=None):
        ...
        if isinstance(cursor, tuple):
            ...
            db = cursor._db()
            self._object_types = db._object_types
            self._modmap = db._modmap
    ...
    def __getitem__(self, key):
        ...
        if not attr[1]:
            ...
            if self._pickle and not isinstance(self._pickle, dict):
                ...
                self._pickle = dbunpickle(self._pickle, self._modmap)
...
copyreg.pickle(ObjectRow, _pickle_ObjectRow, _unpickle_ObjectRow)



PATH_SPLIT = re.compile(r'(\d+)|[_\W]', re.U | re.X)
def split_path(s):
    """
...
class QExpr:
    """
    Flexible query expressions for use with :meth:`~Database.query`
    """
...
class Database:
    ATTR_SIMPLE = ATTR_SIMPLE
    ATTR_SEARCHABLE = ATTR_SEARCHABLE
    ATTR_INDEXED = ATTR_INDEXED
    ATTR_IGNORE_CASE = ATTR_IGNORE_CASE
    ATTR_INVERTED_INDEX = ATTR_INVERTED_INDEX
    ATTR_INDEXED_IGNORE_CASE = ATTR_INDEXED_IGNORE_CASE

    def __init__(self, dbfile, modmap=None, *, loop=None):
        """
    ...
    def _set_dirty(self):
        if self._lazy_commit_interval is not None:
            if self._lazy_commit_timer:
                self._lazy_commit_timer.cancel()
            self._lazy_commit_timer = self._loop.call_later(self._lazy_commit_interval, self.commit)
        if self._dirty:
            return
        self._dirty = True
    ...
    def _unpickle(self, s):
        return dbunpickle(s, self._modmap)


    def _pickle(self, value):
        return dbpickle(value)
    ...
    def register_object_type_attrs(self, type_name, indexes = [], **attrs):
        ...
        :param indexes: a list of tuples where each tuple contains 2 or more
                        registered :attr:`~Database.ATTR_SEARCHABLE` attributes
        ...
        :param attrs: keyword arguments defining the attributes to be
                      ...
                      :attr:`~db.RESERVED_ATTRIBUTES`.  See below for a
        ...
        *searchable* or *simple*.  *Searchable* attributes occupy a column in
        ...
        the form (attr_type, flags, ivtidx, split).
            ...
            * attr_type: the type of the object attribute. For simple attributes
              (:attr:`~Database.ATTR_SIMPLE` in *flags*), this can be any picklable
              type; for searchable attributes (:attr:`~Database.ATTR_SEARCHABLE`
              in *flags*), this must be either *int*, *float*, *str*, *bytes*,
              or *bool*.
            ...
            * ivtidx: name of a previously registered inverted index used for
              ...
              :attr:`~Database.ATTR_INVERTED_INDEX`
        ...
        attributes must be :attr:`~Database.ATTR_SIMPLE`, and of type *list*.
    ...
    def _load_inverted_indexes(self):
        for name, attr, value in self._db_query("SELECT * from inverted_indexes"):
            ...
            if attr == 'objectcount':
                ...
                definition = {}
                for k, v in self._unpickle(value).items():
                    definition[btos(k)] = btos(v)
                self._inverted_indexes[name].update(definition)
    ...
    def _load_object_types(self):
        ...
        for id, name, attrs, idx in self._db_query("SELECT * from types"):
            ...
            # Convert ivtidx and split values to string if they are bytes
            # (i.e. python 2 pickle read in python 3)
            attrs, idx = self._unpickle(attrs), self._unpickle(idx)
            if is_pickle_proto_2:
                copy = {}
                for attr_name, (attr_type, flags, attr_ivtidx, attr_split) in attrs.items():
                    copy[btos(attr_name)] = attr_type, flags, btos(attr_ivtidx), btos(attr_split)
                attrs = copy
            self._object_types[name] = id, attrs, idx


        if sys.hexversion >= 0x03000000 and is_pickle_proto_2:
            self._readonly = True
            log.warning('databases created by Python 2 are read-only until upgrade_to_py3() is called')
    ...
    def _make_query_from_attrs(self, query_type, attrs, type_name):
        ...
        for name, (attr_type, flags, attr_ivtidx, attr_split) in type_attrs.items():
            if flags & ATTR_SEARCHABLE and name in attrs:
                ...
                if isinstance(value, (int, float)) and attr_type in (int, float):
    ...
    def delete_by_query(self, **attrs):
        ...
        :param attrs: see :meth:`~Database.query` for details.
    ...
    def add(self, object_type, parent=None, **attrs):
        ...
        :param object_type: the name of the object type previously created by
                            :meth:`~Database.register_object_type_attrs`.
    ...
    def update(self, obj, parent=None, **attrs):
        ...
        Continuing from the example in :meth:`~Database.add`, consider::
        ...
        .. warning::
           ...
           In the email example from :meth:`~Database.register_object_type_attrs`,
           if the subject attribute is updated by itself, any previously
           indexed terms passed to the keywords attribute (the message body)
           would be discarded after the update.  If updating the subject, the
           caller would be required to pass the message body in the keywords
           attribute again, in order to preserve those terms.
    ...
    def query(self, **attrs):
        ...
        Keyword arguments can be any previously registered :attr:`~Database.ATTR_SEARCHABLE`
        ...
        :type parent: :class:`ObjectRow`, 2-tuple (object_type, object_id), 2-tuple
                      (object_type, :class:`~QExpr`), or a list of those
        ...
        :meth:`~Database.register_object_type_attrs`::
            ...
            [<toolbox.db.ObjectRow object at 0x7f652b251030>]
            >>> # Keyword search requires all keywords
            >>> db.query(keywords=['death', 'blast'])
            [<toolbox.db.ObjectRow object at 0x7f652c3d1f90>]
            ...
            [<toolbox.db.ObjectRow object at 0x7f652b255030>]
        ...
        for ivtidx in self._inverted_indexes:
            ...
            if ivtidx in attrs:
                ...
                if ivtidx_results is None:
                    ...
                    for o in list(ivtidx_results.keys()):
                        if o not in r:
        ...
        for type_name, (type_id, type_attrs, type_idx) in type_list:
            ...
            for attr, value in attrs.items():
                ...
                # type.
                if attr_type in (int, float) and (isinstance(value._operand, (int, float)) or \
                    isinstance(value._operand, str) and value._operand.isdigit()):
                ...
                if isinstance(value._operand, str) and type_attrs[attr][1] & ATTR_IGNORE_CASE:
    ...
    def query_one(self, **attrs):
        """
        Like :meth:`~Database.query` but returns a single object only.
    ...
    def _score_terms(self, terms_list):
        ...
        tostr() to try to decode them intelligently.
    ...
    def get_inverted_index_terms(self, ivtidx, associated = None, prefix = None):
        ...
        :param ivtidx: the name of an inverted index previously registered with
                       :meth:`~Database.register_inverted_index`.
        :type ivtidx: str
    ...
    def get_metadata(self, key, default=None):
        """
        Fetch metadata previously set by :meth:`~Database.set_metadata`.
    ...
    def upgrade_to_py3(self):
        """
        Irreversibly upgrades the database so that Python 3 can write to it,
        after which point Python 2 will be unable to open this database.
        """
