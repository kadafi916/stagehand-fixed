...
# -----------------------------------------------------------------------------
# Copyright 2006-2014 Dirk Meyer, Jason Tackaberry
#
...
def _get_default_by_platform(platform_defaults, global_default):
    best = None
    best_span = 0

    if platform_defaults is not None:
        for platform in platform_defaults:
            m = re.match(platform, sys.platform)
            if not m:
                continue
            span = m.span()[1]
            if span >= best_span:
                best_span = span
                best = platform_defaults[platform]

    if best_span == 0 and global_default is not None:
...
class Base:
    ...
    def __init__(self, name='', desc='', default=None, defaults=None):
        """
        :param name: name of the config item according to its parent's namespace
        :param desc: description of the item
        :param default: the default value to return if none is explicitly set
        :param defaults: an extension of *default*, this is a dictionary mapping
                         platform to a default value.  The key can be a regular
                         expression tested against *sys.platform*.  If there is
                         no match then the *default* kwarg is used instead.
        """
        ...
        self._name = name
        self._desc = _format(tostr(desc))
        self._default = _get_default_by_platform(defaults, default)
        self._monitors = []
    ...
    def _hash(self, values=True):
        ...
        """
        value = repr(self._stored_value) if values else ''
        return hashlib.md5(tobytes(repr(self._desc) + repr(self._default) + value)).hexdigest()
...
class VarProxy(Base):
    ...
    def __getattribute__(self, attr):
        if attr == '__class__':
            ...
            return super().__getattribute__("_class")
        elif attr in ('add_monitor', 'remove_monitor', 'parent'):
            # Attributes we proxy.
            return getattr(super().__getattribute__('_item'), attr)
...
class Var(Base):
    ...
    unicode, or bool.
    """
    def __init__(self, name='', type='', desc='', default=None, defaults=None, scramblekey=None):
        """
        :param scramblekey: An optional key to obfuscate (not encrypt!) the
                            config value when storing.  Default values aren't
                            scrambled.
        """
        super().__init__(name, desc, default, defaults)
        if type == '':
            # create type based on default
            if self._default == None:
                raise AttributeError('define type or default')
            type = self._default.__class__

        self._type = type
        self._scramblekey = scramblekey
        # The actual value as managed by the _value property.
        self._default = self._unscramble(self._default)
        self._real_value = self._unscramble(self._default)


    def _scramble(self, value):
        """
        This isn't encryption and we so don't call it encryption.  The sole
        purpose of scrambling is to prevent shoulder surfers from being able
        to see at a glance sensitive items like passwords.

        Obviously this is a DRM-style problem and therefore intractable. The
        intention isn't security but obfuscation.  Arguably even having a salt
        is overkill, but the idea there is to not easily be able to correlate
        different config items that may have the same value.
        """
        if not self._scramblekey:
            return value

        # Pad the value up to 8 byte boundaries.
        size = (len(value) // 8 + 1) * 8
        padded = value.encode('utf8') + b'\x00' * (size - len(value))
        # Hash a newly generated salt and the scramble key together to create
        # a kind of session key for this config item.  The session key just
        # loops as much as needed to fit the size of the padded value.
        salt = os.urandom(8)
        key = itertools.cycle(hashlib.sha512(salt + self._scramblekey.encode('utf8')).digest())
        # We just do a simple xor because after all security is not the
        # intention.
        scrambled = bytes(c ^ next(key) for c in padded)
        return b'{scrambled}' + base64.b64encode(salt + scrambled)


    def _unscramble(self, value):
        if not self._scramblekey or not value.startswith('{scrambled}'):
            return value
        scrambled = base64.b64decode(value[11:])
        salt, padded = scrambled[:8], scrambled[8:]
        key = itertools.cycle(hashlib.sha512(salt + self._scramblekey.encode('utf8')).digest())
        return bytes(c ^ next(key) for c in padded).decode('utf8').rstrip('\x00')


    @property
    def _stored_value(self):
        return self._scramble(self._value)
    ...
    def _value(self, value):
        if self._cow_source is not None:
            ...
            self._cow_source = None
        self._real_value = self._unscramble(value)


    def copy(self, copy_on_write=False):
        copy = super(Var, self).copy(copy_on_write)
        copy._type = self._type
        copy._scramblekey = self._scramblekey
    ...
    def _hash(self, values=True):
        ...
        Returns a hash of the config item.
        """
        hashdata = super(Var, self)._hash(values) + repr(self._type) + (self._scramblekey or '')
        return hashlib.md5(tobytes(hashdata)).hexdigest()
    ...
    def _stringify(self, print_desc=True):
        ...
        if self._value == self._default:
            value = '<default: %s>' % (self._value if self._value != '' else '(empty string)')
        else:
            value = self._stored_value
        return '%s%s = %s' % (desc, self._get_fqname(), tostr(value))
...
class Group(Base):
    ...
    A config group.
    """
    def __init__(self, schema, desc='', name='', desc_type='default'):
        self._dict = {}
        self._vars = []
    ...
    def _hash(self, values=True):
        ...
        Returns a hash of the config item.
        """
        hash = hashlib.md5(tobytes(super()._hash(values)))
        for name in self._vars:
            hash.update(tobytes(name + self._dict[name]._hash(values)))
...
class Container(Base):
    ...
    def _hash(self, values=True):
        ...
        Returns a hash of the config item.
        """
        hash = hashlib.md5(tobytes(super(Container, self)._hash(values)))
        for name, var in self._vars():
            hash.update(tobytes(repr(name) + var._hash(values)))
        return hash.hexdigest()
...
class Dict(Container):
    ...
    A config dict.
    """
    def __init__(self, schema, desc='', name='', type=str, defaults={}):
        self._dict = {}
        super().__init__(name, desc, schema, type, defaults.items())
...
class Config(Group):
    ...
    """
    def __init__(self, schema, desc='', name='', module=None):
        super().__init__(schema, desc, name)
        self.signals = Signals('reloaded')
        self.filename = None
        self._bad_lines = []
    ...
    def _hash(self, values=True):
        ...
        Returns a hash of the config item.
        """
        return hashlib.md5(tobytes(super(Config, self)._hash(values) + repr(self._bad_lines))).hexdigest()
    ...
    def copy(self, copy_on_write=False):
        ...
        copy._inotify = None
        copy._watch_timer = None
        copy._autosave_timer = None
        return copy
    ...
    def save(self, filename=None, force=False):
        ...
        if self._bad_lines:
            f.write('\n# =========================================================\n'
                    ...
                    '# =========================================================\n')
        f.write(tostr(self._stringify(), encoding) + '\n')
        if self._bad_lines:
            f.write('\n\n\n'
                    '# *************************************************************\n'
        ...
        # Atomic rename on both Windows and *nix, but requires Python 3.3
    ...
    def load(self, filename=None, sync=False):
        ...
        :param filename: filename to load values from; if None, will use the
                          :attr:`~config.Config.filename` property.
        :type filename: str
        ...
        If no filename has been previously set with the :attr:`~config.Config.filename`
        ...
        for line in f.readlines():
            ...
            line = tostr(line, local_encoding)
    ...
    def autosave(self):
        ...
        :meth:`~config.Config.load` or defined by the
        :attr:`~config.Config.filename` property) 5 seconds after the last
    ...
    def autosave(self, autosave):
        if autosave and not self._autosave:
            self.add_monitor(self._config_changed_cb)
        elif not autosave and self._autosave:
            self.remove_monitor(self._config_changed_cb)
            self._stop_autosave_timer()
    ...
    def _config_changed_cb(self, name, oldval, newval):
        if self.filename:
            if not self._autosave_timer:
                # TODO: connect self.save to exit
                # main.signals['exit'].connect(self.save)
                pass

            self._stop_autosave_timer()
            # Start/restart the timer to save in 5 seconds.
            self._autosave_timer = asyncio.get_event_loop().call_later(5, self.save)
        ...
        if not watch and self._watching:
            ...
            try:
                if self._inotify:
                    ...
                    signal.connect(self._file_changed)
                else:
                    raise OSError
            except OSError:
                # Adding watch failed, use timer to wait for file to appear.
                self._watch_timer = asyncio.get_event_loop().call_later(3, self._check_file_changed)
            self._watching = True
    ...
    def _file_changed(self, mask, path, target):
        if mask & INotify.MODIFY:
            ...
            asyncio.get_event_loop().call_later(0.1, self._check_file_changed)
...
def get_type(var):
    ...
    For non-scalar types, the return value will be one of :class:`~config.List`,
    :class:`~config.Dict`, or :class:`~config.Group`.
    """
    if isinstance(var, VarProxy):
...
def get_schema(var):
    ...
    The schema is another config object (:class:`~config.Var`,
    :class:`~config.List`, :class:`~config.Dict`, or
    :class:`~config.Group`).

        >>> config.get_description(cfg.movies)
        'Your favorite movies.'
        >>> schema = config.get_schema(cfg.movies)
        >>> config.get_description(schema)
        'A movie name.'
        >>> config.get_type(schema)
        <type 'str'>
    """
    try:
        return var._schema
